...
public class DynamicDataSource extends AbstractRoutingDataSource {
    ...
    private static Gson                             GSON                  = new GsonBuilder().setLongSerializationPolicy(LongSerializationPolicy.STRING).disableHtmlEscaping()
                                                                                  .create();
    ...
    protected List<DynamicDataSourceUpdateListener> listeners             = new ArrayList<DynamicDataSourceUpdateListener>();
    ...
    public void afterPropertiesSet() {
        ...
        if (subscriber != null && needRemote) {
            subscriber.addListeners(new SubscriberListener() {
                @Override
                public boolean init(Topic topic) {
                    return true;
                }

                @Override
                public boolean reload(Topic topic) {
                    updateDataousrce(MessageAction.Message_Action_Reload);
                    return false;
                }

                @Override
                public boolean update(Topic topic) {
                    updateDataousrce(MessageAction.Message_Action_Update);
                    return false;
                }
    ...
    private boolean updateDataousrce(MessageAction ma) {
        ...
        for (Entry<String, Map<String, String>> e : map.entrySet()) {
            ...
            if (listeners != null) {
                for (DynamicDataSourceUpdateListener listener : listeners)
    ...
    public void addDynamicDataSourceUpdateListener(DynamicDataSourceUpdateListener l){
        listeners.add(l);
    }
    
    public void removeDynamicDataSourceUpdateListener(DynamicDataSourceUpdateListener l){
        listeners.remove(l);
    }
